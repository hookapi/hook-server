(function() {
  ...
  unionfs.prototype.start = function(cb) {
    if (!fs.existsSync(this.tmp_base)) fs.mkdirSync(this.tmp_base, '0777');
    if (!fs.existsSync(this.tmp_base_os)) fs.mkdirSync(this.tmp_base_os, '0777');
    if (!fs.existsSync(this.tmp_base_app)) fs.mkdirSync(this.tmp_base_app, '0777');
    if (!fs.existsSync(this.tmp_base_app_app)) fs.mkdirSync(this.tmp_base_app_app, '0777');
    if (!fs.existsSync(this.tmp_base_rw)) fs.mkdirSync(this.tmp_base_rw, '0777');
    if (!fs.existsSync(this.target_path)) fs.mkdirSync(this.target_path, '0777');
    var unionfs_opts = 'cow,nonempty,default_permissions,allow_other,use_ino,suid,chroot=' + this.tmp_base;
