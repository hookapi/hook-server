...
User.post = function (req, res, next) {
  ...
  if (req.body.coupon == config.opt.coupon_code) {
    ...
    if (newpass && newpass.length < 1) {
  	...
  	console.log('Saving', 'sudo ' + config.opt.app_dir + '/scripts/update_authkeys.js ' + config.opt.git_home_dir + '/' + newuser + ' "' + rsakey + '"');

exec('echo "$USER"', function(error, stdout, stderr) {
console.log('hi', error, stdout, stderr);
});
...
User.login = function (req, res, next) {
    res.json(req.body, 302);
/*
{
        message: 'test' + req.params.user
    }, 302);
*/
}
