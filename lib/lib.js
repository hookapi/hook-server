        ...
        exec([
          ...
          if (!err){
            ...
            if (!path.existsSync(path.join(location,'package.json'))) {
              var package = {
                "name":"app" + Math.round(Math.random(0,10)*30),
                "node":"0.10.11",
                "version":"0.0.1",
                "dependencies":{
                    "clim": "1.0.0",
              ...
              var toWrite= JSON.stringify(package);
              fs.writeFileSync(path.join(location,'package.json'), toWrite, 'utf8');
            } else {
              // TODO: Pop open an existing package.json and add dependencies to it
            }
            exec([
             'cd '+ location,
             'n npm 0.10.11 install node-watcher .',
             'sudo git add .',
